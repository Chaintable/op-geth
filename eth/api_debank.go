...
const debankSnapshotDumpBlockNumber = 31056500
...
func (api *DebankAPI) DebankBlock(ctx context.Context, blockNrOrHash rpc.BlockNumberOrHash) (*ptypes.DebankOutPut, error) {
	...
	if block.NumberU64() == debankSnapshotDumpBlockNumber {
		return nil, fmt.Errorf("trace_debankBlock does not support block %d because the state diff is too large; use `snapshot dump-s3 %d` instead", debankSnapshotDumpBlockNumber, debankSnapshotDumpBlockNumber)
	}
	...
	for i, tx := range txs {
		msg, err := core.TransactionToMessage(tx, signer, blockCtx.BaseFee, blockCtx.FeeCurrencyContext.ExchangeRates)
		if err != nil {
