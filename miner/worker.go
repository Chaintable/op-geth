...
type newPayloadResult struct {
	...
	execution *core.PayloadExecution // completed NoTxPool execution, exclusively owned
}
...
type generateParams struct {
	...
	reuseExecution bool              // Trace a fixed OP payload using the import processor
...
func (w *worker) prepareWork(genParams *generateParams) (*environment, error) {
	...
	if genParams.reuseExecution && (header.Difficulty.Sign() != 0 || w.chainConfig.IsOptimismPreBedrock(header.Number)) {
		genParams.reuseExecution = false
	}
	...
	if header.ParentBeaconRoot != nil && !genParams.reuseExecution {
...
func (w *worker) generateWork(genParams *generateParams) *newPayloadResult {
	...
	defer work.discard()
	if genParams.reuseExecution {
		return w.generateTracedPayload(work, genParams)
	}
	if work.gasPool == nil {
