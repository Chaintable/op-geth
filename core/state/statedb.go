...
func (s *StateDB) commitAndFlush(block uint64, deleteEmptyObjects bool, noStorageWiping bool, deriveCodeFields bool) (*StateUpdate, error) {
	...
	if s.OnCommit != nil && !ret.Empty() {
		// Preserve the Pipeline wire format while the state backend uses typed
		// account and storage updates internally.
		accounts, accountsOrigin, storages, storagesOrigin := ret.EncodeMPTState()
		destructs := make(map[common.Hash]struct{})
		for hash, account := range accounts {
			if account == nil {
				destructs[hash] = struct{}{}
				delete(accounts, hash)
			}
		}
		codes := make(map[common.Hash][]byte, len(ret.Codes))
		for _, code := range ret.Codes {
			codes[code.Hash] = code.Blob
		}
		s.OnCommit(ret.OriginRoot, ret.Root, destructs, accounts, accountsOrigin, storages, storagesOrigin, codes)
	}
