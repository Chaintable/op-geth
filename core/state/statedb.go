...
func (s *StateDB) commitAndFlush(block uint64, deleteEmptyObjects bool, noStorageWiping bool, deriveCodeFields bool) (*StateUpdate, error) {
	...
	// Emit the state diff to the pipeline tracer only after disk persistence
	// succeeds, so consumers (e.g. Kafka) never observe state that was rolled
	// back due to a commit failure.
	if !ret.empty() && s.OnCommit != nil {
