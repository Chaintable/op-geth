...
func (s *StateDB) RawDump2(opts *DumpConfig, dataDir string) Dump {
	...
	log.Info("RawDump2 starting", "dataDir", dataDir, "progressDir", progressDir)

	progress, err := s.loadProgressFile(progressFile)
	if err != nil {
		progress = &ProgressDump{}
		log.Info("Starting new dump, no existing progress", "err", err)
	} else {
		log.Info("Loaded existing progress", "exported", progress.ExportedCount, "batches", progress.BatchCount, "complete", progress.IsComplete)
	}
	...
	if progress.IsComplete {
		...
		log.Info("Returning completed dump from batches", "totalAccounts", len(allAccounts))
		return Dump{
			Root:     progress.Root,
	...
	for {
		...
		if len(batchDump.Accounts) > 0 {
			...
			if err := s.saveBatchFile(batchDir, progress.BatchCount, batchDump.Accounts); err != nil {
				log.Error("Failed to save batch file", "batch", progress.BatchCount, "batchDir", batchDir, "err", err)
				return Dump{}
			}
			log.Info("Saved batch file", "batch", progress.BatchCount, "accounts", len(batchDump.Accounts))
		}
		...
		if err := s.saveProgressFile(progressFile, progress); err != nil {
			log.Error("Failed to save progress", "progressFile", progressFile, "err", err)
		}
	...
	log.Info("RawDump2 completed", "totalBatches", progress.BatchCount, "totalAccounts", len(allAccounts))
	return Dump{
		Root:     progress.Root,
		Accounts: allAccounts,
	}
