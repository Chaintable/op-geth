...
func (bc *BlockChain) writeBlockAndSetHead(block *types.Block, receipts []*types.Receipt, logs []*types.Log, state *state.StateDB, emitHeadEvent bool) (status WriteStatus, err error) {
	...
	if status == CanonStatTy {
		...
		if tracer.NodeXPusher != nil && leader.GlobalManager.IsLeader() && lastPushedBlock != nil && lastPushedBlock.BlockNumber <= block.NumberU64() {
...
func (bc *BlockChain) SetCanonical(head *types.Block) (common.Hash, error) {
	...
	if tracer.NodeXPusher != nil && leader.GlobalManager.IsLeader() && lastPushedBlock != nil && lastPushedBlock.BlockNumber <= head.NumberU64() {
		_, dropBlocks, newBlocks := bc.getCommonAncestor(*lastPushedBlock, ptypes.BlockContext{
